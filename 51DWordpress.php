...
if(get_option('show_51DMeta')){ include_once('51Degrees.mobi.metadata.php'); }
if(get_option('enable_51DUDP')){ include_once('51Degrees.mobi.usage.php'); }

add_action('admin_head', 'fiftyone_degrees_update_javascript');

function fiftyone_degrees_update_javascript() {
?>
<script type="text/javascript" >
  function fiftyone_degrees_StartUpdate()
  {
  jQuery(document).ready(function($) {
  jQuery('.fiftyone_degrees_updating_text').html("Updating now, please wait...");
  ...
  jQuery('.fiftyone_degrees_updating_text').html("");
  });
...
function my_action_callback() {
  ...
  $premiumLicenseKey = fiftyone_degrees_GetLicenses($dir);
  $keysValid = true;
  ...
  if($keysValid)
  {
      ...
      {
          fiftyone_degrees_RemoveOldFiles($dir);
          $url = 'https://51degrees.mobi/devicedata/getdelta.ashx?LicenseKeys='.$premiumLicenseKey;
          fiftyone_degrees_GetPHPFileArrays($dir, $phpCurrent);
          $updateFile = fiftyone_degrees_Download($url, $phpCurrent, $dir);
          ...
          {
              fiftyone_degrees_DoUpdate($phpCurrent, $updateFile, $url, $dir);
...
function fiftyone_degrees_GetLicenses($dir)
{
    $licenses = array();
...
function fiftyone_degrees_GetPhpFileArrays($dir, &$phpCurrent)
{
    ...
    if($currentHash == "")
    {
        fiftyone_degrees_WriteUpdateHashFile($dir, $currentHash);
    }
...
function fiftyone_degrees_WriteUpdateHashFile($dir, &$currentHash)
{
    $dirlengths = ""; $filename = $dir.'/UpdateHash.txt';
    fiftyone_degrees_PhpFileArraysProcessor($dir, $currentHash, $dirlengths, false);
    $fh = fopen($filename, 'w');
    fwrite($fh, $currentHash);
    fclose($fh);
    $dirlengths = fiftyone_degrees_ShortenLengthsVar($dirlengths);
...
function fiftyone_degrees_PhpFileArraysProcessor($dir, &$currentHash, &$dirlengths, $isSubdir)
{
    foreach(glob($dir.'/*') as $file) {
        if(is_dir($file))
        {
            fiftyone_degrees_PhpFileArraysProcessor($file, $currentHash, $dirlengths, true);
        }
        else
        {
            $fileWithDir = substr($file, 13);
            if($isSubdir === true ||
                ($fileWithDir == "51Degrees.mobi.php" || $fileWithDir == "51Degrees.mobi.usage.php" || $fileWithDir == "51Degrees.mobi.metadata.php" || $fileWithDir == "License.txt"))
            {
...
function fiftyone_degrees_ShortenLengthsVar($dirlengths)
{
...
function fiftyone_degrees_DoUpdate($phpCurrent, &$updateFile, $url, $dir)
{
    fiftyone_degrees_DoubleCheck($phpCurrent, $updateFile, $url, $dir);
    ...
    foreach($linevars as $value)
    {
        ...
        if(trim($vals[0]) != "[!DELETE")
        {
            ...
            $res = fiftyone_degrees_CreateUpdateFile($dir.'/'.$vals[1], implode('|', $text));
            if($res != "")
            {
                fiftyone_degrees_RollbackUpdate($dir);
    ...
    try {
        fiftyone_degrees_DeleteMarked($deleteFiles);
    } catch (Exception $e) {
...
function fiftyone_degrees_DoubleCheck($phpCurrent, &$updateFile, $url, $dir)
{
    $checking = fiftyone_degrees_Download($url, $phpCurrent, $dir);
...
function fiftyone_degrees_CreateUpdateFile($fileDirectory, $fileContents)
{
    try {
...
function fiftyone_degrees_DeleteMarked($deleteFiles)
{
...
function fiftyone_degrees_RollbackUpdate($dir)
{
    foreach(glob($dir.'/*') as $file) {
        if(is_dir($file))
        {
            fiftyone_degrees_RollbackUpdate($file);
        }
        else
        {
...
function fiftyone_degrees_RemoveOldFiles($dir)
{
    foreach(glob($dir.'/*') as $file) {
        if(is_dir($file))
        {
            fiftyone_degrees_RemoveOldFiles($file);
        }
        else
        {
            if(substr($file, strlen($file) - 3, strlen($file)) == "old")
...
<form method="post">
  <p>
    <input type="checkbox" name="show_51DMeta" <?php echo $show_51DMeta_checked; ?> /> Enable meta data. <a href="http://51degrees.mobi/Support/Documentation/PHP.aspx#MetaData">Learn More</a>
  </p>
  <p>
    <input type="checkbox" name="enable_51DUDP" <?php echo $enable_51DUDP_checked; ?> /> Share usage data with 51Degrees.mobi. <a href="http://51degrees.mobi/Support/FAQs/UsageData.aspx">Learn More</a>
  </p>
  <p>
    <input type="submit" name="submit" value="Sumbit" />
  </p>
  <p class="fiftyone_degrees_updating_text">
    <input type="button" value="Update to Latest" onclick="fiftyone_degrees_StartUpdate()" />
  </p>
</form>
<form method="post">
